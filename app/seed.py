...
def apply_roles(conn: psycopg.Connection) -> None:
    try:
        ...
        # Managed hosts (e.g. Neon) reject the local demo password at the
        # control plane, outside plpgsql exception handling. The app only
        # needs SET ROLE, which works with a NOLOGIN role.
        conn.execute(NOLOGIN_ROLES_SQL)
